...
export class core 
{
    ...
    async listen(port)
    {
        ...
        this.util = new util();
        //PLUGIN KAPISI *****************************/
        //PLUGINLER ASENKRON YUKLENDIGI ICIN ERKEN BAGLANAN CLIENT PLUGIN LISTENERLARINI ALAMIYOR.
        //MIDDLEWARE CONNECTION EVENTINDEN ONCE CALISIR, PLUGINLER HAZIR OLANA KADAR BAGLANTI BEKLETILIYOR.
        this.pluginsReady = false;
        this.socket.use(async (pSocket,next) => 
        {
            while(!this.pluginsReady)
            {
        ...
        let tmpPluginLoad = [];
        //PLUGIN YAPISI ******************************/
        fs.readdirSync(this.root_path + '/plugins').forEach(file => 
        {   
            if(path.extname(file).toLowerCase() == '.js')
            {
                try 
                {
                    ...
                    //************************* */
                    tmpPluginLoad.push(import(tmpPath + '/plugins/' + file).then(module =>
                    {
        ...
        //*******************************************/
        //OZEL PLUGIN YAPISI ************************/
        if (fs.existsSync(this.root_path + '/plugins/private')) 
        {
            fs.readdirSync(this.root_path + '/plugins/private').forEach(file => 
            {   
                if(path.extname(file).toLowerCase() == '.js')
                {
                    try 
                    {
                        ...
                        //************************* */
                        tmpPluginLoad.push(import(tmpPath + '/plugins/private/' + file).then(module =>
                        {
                            Object.keys(module).forEach(element => 
                            {
                                this.plugins[element] = module[element];
        ...
        //*******************************************/
        //BIR PLUGIN IMPORTU ASILI KALIRSA SUNUCU HIC BAGLANTI KABUL ETMESIN DIYE 10 SN EMNIYET SURESI VAR.
        await Promise.race(
        [
            Promise.allSettled(tmpPluginLoad),
            new Promise(resolve => setTimeout(resolve,10000))
        ]);
        this.pluginsReady = true;
        console.log('Plugins loaded (' + tmpPluginLoad.length + ' file) - socket connections opened');
    }   
    //#region  "EVENT"
    on(pEvt, pCallback) 
    {
...
export class sql 
{
    ...
    connEvt(pSocket)
    {
        ...
        pSocket.on('sql',async (pQuery,pCallback) =>
        {
            ...
            //SQL SORGUSU ÇALIŞMADAN ÖNCE CORE EVENT I TETIKLENIYOR.
            this.core.emit('onSqlExecuting',{executingQuery:pQuery})
            //SQL SORGUSU ÇALIŞTIRILIYOR.
            let tmpResult = await this.executePool(pQuery,pSocket.conn.id)
            //SQL SORGUSU ÇALIŞTIKTAN SONRA CORE EVENT I TETIKLENIYOR.
            this.core.emit('onSqlExecuted',{executedQuery:pQuery,executedResult:tmpResult})
...
export class auth
{
    ...
    getUserList()
    {
        return new Promise(async resolve =>
        {
            let TmpQuery =
            {
                query:  "SELECT CODE,NAME,LAST_NAME,CARDID,(SELECT CAST( CAST( PWD as XML ).value('.','varbinary(max)') AS varchar(max) )) AS PWD,ROLE,USER_APP FROM USERS WHERE STATUS = 1",
            }
...
export class util 
{
    ...
    toBase64(pText)
    {
        return Buffer.from(pText).toString('base64')
    }
    getVersion()
    {
        try
