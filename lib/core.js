        ...
        if(typeof pQuery.param != 'undefined')
        {
            for(let i = 0;i < pQuery.param.length;i++)
            {
                ...
                {                    
                    ...
                    this.input(pQuery.param[i].split(":")[0],mssql.DateTime,date);
                    // let tmpQuery = {...pQuery};
...
export class auth
{
    ...
    getUserList()
    {
        return new Promise(async resolve =>
        {
            let TmpQuery =
            {
                query:  "SELECT CODE,NAME,CARDID,(SELECT CAST( CAST( PWD as XML ).value('.','varbinary(max)') AS varchar(max) )) AS PWD FROM USERS",
            }
