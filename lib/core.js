...
export default class core
{
    ...
    {                
        ...
        this.log.msg('Licence checking...','Licence');
        await this.licence.check();
        if (this.licence.status)
        {
            this.log.msg('Licence check successful.','Licence');
        }
        else
        {
            this.log.msg('Invalid licence.','Licence');
        }
...
export class io_manager
{
    ...
    io_connection(pSocket)
    {
        ...
        {
            if(core.instance.licence.user_count < core.instance.io_manager.clients().length)
            {
...
export class licence
{
    ...
    {
        this.socket = null;
        this.app = core.instance.config.app;
    ...
    check()
    {
        return new Promise(resolve =>
        {
            ...
            this.socket = new client('http://localhost:8081',{'timeout':2000, 'connect timeout': 2000})
            this.socket.on('connect', () => 
            {
                this.socket.emit('licence_check',{MacId:this.macid,App:this.app},async (pData) =>
                {      
