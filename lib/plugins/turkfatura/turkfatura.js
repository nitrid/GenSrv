...
export default class turkfatura
{
    ...
    E_FATURA_OLUSTURMA()
    {
        return new Promise(async resolve =>
        {
            ...
            let MasterData = await this.Query("SELECT * FROM [TERP_E-FATURA_MASTER] WHERE STATUS = 0 ","","")
            ...
            for (let i = 0; i < MasterData.length; i++) 
            {
                ...
                await this.Query("UPDATE [TERP_E-FATURA_MASTER] SET STATUS = @STATUS,CALLBACK = @CALLBACK WHERE GUID = @GUID",["STATUS:int","CALLBACK:string|500","GUID:string|50"],[status,JSON.stringify(JSON.parse(callback.body)),MasterData[i].GUID])
            }
    ...
    E_FATURA_LISTELE()
    {
        return new Promise(async resolve =>
        {
            ...
            let BASLANGIC_TARIHI = "2021-07-16T04:25:03Z"
            let BITIS_TARIHI = "2021-07-16T19:25:03Z"
